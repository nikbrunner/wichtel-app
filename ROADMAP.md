## TODO

## Completed

- [x] fix: title and favicon
- [x] refactor: At least 3 participants
- [x] feat: Admin Accounts
  - [x] Events Overview Dashboard
  - [x] Participant Links Management
  - [x] Draw Stats
  - [x] Date-based draw results visibility
  - [x] Event date tracking
  - [x] Link regeneration with modal
  - [x] RLS Security

## Open Goals

- [ ] feat: Internationalization
- [ ] feat: light/dark mode
- [ ] test: What happens if we add more participants during a running event?
- [ ] feat: A participant can optinally provide a list of interests
  - Maybe a pre-draw date until adding new participants is possible and interests can also be added
  - Then everything is locked in and everybody can draw
- [ ] feat: event page needs to be redone
- [ ] feat: Add zod for validation
- [ ] refactor: cleanup
- [ ] fix: sign-in/up: fixed height

---

## 2025.12.02 - Tuesday

- [x] Manual phase 2 trigger when all participants have entered interests
- [x] fix: loading states for buttons
  - Shows loading state when clicked but reverts to original state before submission
- [x] Interrests: Handle open entry in input field
- [x] Improve texts for explaining phases
- [x] style: create Logo in Figma
- [x] Send out invites

## 2025.11.30 - Sunday

- [x] fix: db
- [x] chore: bring .env into 1P

## 2025.11.25 - Tuesday

- [x] style: Better styling (new framework)
- [x] feat: custom stylings
- [x] style: Better font
- [x] feat: delete events
- [x] feat: delete participants
- [x] feat: add participants to existing events
- [x] sign-in/up: via Modal
- [x] feat: landing page when not logged in
